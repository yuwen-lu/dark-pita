...
export default {
  ...
  methods: {
    ...
    togglePopup(event, value, index) {
      ...
      // Calculate available space for the popup on the right and left sides of the target
      let spaceRight = this.overlayWidth - target.right - margin;
      let spaceLeft = target.left - margin;

      if (spaceRight > popupWidth || spaceRight >= spaceLeft) {
        this.popupX = target.right + margin;
      } else {
        this.popupX = target.left - margin - popupWidth;
      }

      // Calculate available space for the popup above and below the target
      let spaceBelow = this.overlayHeight - target.bottom - margin;
      let spaceAbove = target.top - margin;

      if (spaceBelow > popupHeight || spaceBelow >= spaceAbove) {
        this.popupY = target.bottom + margin;
      } else {
        this.popupY = target.top - margin - popupHeight;
      }

      // Ensure the popup stays within the boundaries of the overlay
      this.popupX = Math.max(
        0,
        Math.min(this.popupX, this.overlayWidth - popupWidth)
      );
      this.popupY = Math.max(
        100,
        Math.min(this.popupY, this.overlayHeight - popupHeight)
      );
